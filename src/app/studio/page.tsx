...
function LockScreen({ onUnlock }: { onUnlock: () => void }) {
  ...
  return (
    <>
      <CustomCursor />
      <main className="relative min-h-screen flex flex-col items-center justify-center bg-black text-white overflow-x-hidden font-[family-name:var(--font-geist-sans)] px-6">
        <AmbientGlow />
        <motion.div
              ...
              <GlassConsole className="p-8">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className={`text-5xl font-bold tabular-nums ${getScoreColor(analysis.score)}`}>
                      {analysis.score}
                    </span>
                    ...
                    {learningPlan && (
                      <div className="mt-6 space-y-4">
                        {learningPlan.plans.map((plan, i) => {
                          ...
                          return (
                            <div key={plan.skill} className="border border-white/5 rounded-xl p-4">
                              <div
                                    ...
                                    <div className="pt-3">
                                          ...
                                          >
                                            ...
                                            {step.resources?.length > 0 && (
                                              <ul className="ml-4 space-y-1 mb-1.5">
                                                {step.resources.map((res, k) => (
                                                  ...
                                                  >
                                                    <span>{RESOURCE_EMOJI[res.type] ?? "🔗"}</span>
                                            ...
                                            {step.aiPrompt && (
                                              <div className="ml-4 flex items-start gap-2">
                                                ...
                                                <button
                                                  onClick={() =>
                                                    handleCopyPrompt(step.aiPrompt, `${i}-${j}`)
                                                  }
                                      ...
                                      <div className="flex items-start justify-between gap-2">
                                        <p className="text-xs text-zinc-200 flex-1 leading-relaxed">
                                          <span className="text-indigo-400">Resume bullet: </span>
                                          {plan.resumeBullet}
                                        </p>
                                        <button
                                          onClick={() => handleCopyBullet(plan.resumeBullet, plan.skill)}
                ...
                <div className="border-t border-white/5 pt-8 mb-8">
                  ...
                  <motion.button
                    whileHover={{ scale: !generateResume && !generateCoverLetter ? 1 : 1.01 }}
                    whileTap={{ scale: !generateResume && !generateCoverLetter ? 1 : 0.98 }}
                    onClick={handleGenerateDocs}
                    disabled={(!generateResume && !generateCoverLetter) || generatingDocs}
                    ...
                    <div className="mt-6 space-y-6">
                      {documents.resume && (
                        <div>
                          ...
                          <div className="flex gap-2 mt-2">
                            <button
                              onClick={() => handleCopyDoc(documents.resume!, "resume")}
                              className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-300 transition-colors px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg"
                            >
                              {copiedDoc === "resume" ? (
                                <Check size={11} className="text-green-400" />
                      ...
                      {documents.coverLetter && (
                        <div>
                          ...
                          <div className="flex gap-2 mt-2">
                            <button
                              onClick={() => handleCopyDoc(documents.coverLetter!, "coverLetter")}
                              className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-300 transition-colors px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg"
                            >
